...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    // Force initial sizing with retry mechanism
    // This handles dynamic import timing issues
    const initializeSize = () => {
      const w = mount.clientWidth;
      const h = mount.clientHeight;
      if (w > 1 && h > 1) {
        // Valid dimensions, force resize
        lastSizeRef.current = { width: 0, height: 0, dpr: 0 }; // Reset to force update
        setSizeNow();
      } else {
        // Retry if dimensions not ready
        requestAnimationFrame(initializeSize);
      }
    };
